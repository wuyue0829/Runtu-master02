...
public class CaptureManager {
    ...
    private              int     orientationLock        = ActivityInfo
            .SCREEN_ORIENTATION_UNSPECIFIED;
    private static final String  SAVED_ORIENTATION_LOCK =
    ...
    private final CameraPreview.StateListener stateListener = new
     ...
     *
     * @param intent
     *         the intent containing the scanning preferences
     * @param savedInstanceState
     *         saved state, containing orientation lock
     */
    public void initializeFromIntent(Intent intent, Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            // If the screen was locked and unlocked again, we may start in a
            // different orientation
            // (even one not allowed by the manifest). In this case we
            // restore the orientation we were
            // previously locked to.
            this.orientationLock = savedInstanceState.getInt
                    (SAVED_ORIENTATION_LOCK, ActivityInfo
                            .SCREEN_ORIENTATION_UNSPECIFIED);
        }

        if (intent != null) {
            if (orientationLock == ActivityInfo
                    .SCREEN_ORIENTATION_UNSPECIFIED) {
                // Only lock the orientation if it's not locked to something
                // else yet
                boolean orientationLocked = intent.getBooleanExtra(Intents
                        .Scan.ORIENTATION_LOCKED, true);
            ...
            if (intent.hasExtra(Intents.Scan.TIMEOUT)) {
                ...
                handler.postDelayed(runnable, intent.getLongExtra(Intents
                        .Scan.TIMEOUT, 0L));
            }

            if (intent.getBooleanExtra(Intents.Scan.BARCODE_IMAGE_ENABLED,
                    false)) {
    ...
    protected void lockOrientation() {
        ...
        if (this.orientationLock == ActivityInfo
                .SCREEN_ORIENTATION_UNSPECIFIED) {
            // Adapted from http://stackoverflow.com/a/14565436
            Display display = activity.getWindowManager().getDefaultDisplay();
            int rotation = display.getRotation();
            int baseOrientation = activity.getResources().getConfiguration()
                    .orientation;
            int orientation = 0;
    ...
    private void openCameraWithPermission() {
        if (ContextCompat.checkSelfPermission(this.activity, Manifest
                .permission.CAMERA)
     ...
     *         a path to an exported file of the Barcode Image, can be null.
     * @return the Intent
     */
    public static Intent resultIntent(BarcodeResult rawResult, String
            barcodeImagePath) {
        ...
        if (metadata != null) {
            ...
            Number orientation = (Number) metadata.get(ResultMetadataType
                    .ORIENTATION);
            if (orientation != null) {
    ...
    /**
     * Save the barcode image to a temporary file stored in the application's
    ...
    private String getBarcodeImagePath(BarcodeResult rawResult) {
        ...
        if (returnBarcodeImagePath) {
            ...
            try {
                File bitmapFile = File.createTempFile("barcodeimage", ".jpg",
                ...
                Log.w(TAG, "Unable to create temporary file and store bitmap!" +
                        " " + e);
            }
        }
    ...
    protected void displayFrameworkBugMessageAndExit() {
        ...
        builder.setTitle("权限");
        builder.setMessage("请在设置里面授予相机权限,如果失败,请重启手机");
        builder.setPositiveButton(R.string.zxing_button_ok, new
