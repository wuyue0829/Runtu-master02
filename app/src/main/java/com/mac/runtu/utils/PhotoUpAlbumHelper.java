		...
		if (cur.moveToFirst()) {
			...
			do {
				if (cur.getString(photoPathIndex).substring(
						...
						.replaceAll(" ", "").length()<=0)
				{
					LogUtils.d(TAG, "出现了异常图片的地址：cur.getString(photoPathIndex)="+cur.getString(photoPathIndex));
					LogUtils.d(TAG, "出现了异常图片的地址：cur.getString(photoPathIndex).substring="+cur.getString(photoPathIndex)
...
//					imageItem.setThumbnailPath(thumbnailList.get(_id));
					...
					LogUtils.i(TAG, "PhotoUpAlbumHelper类中 的——》path="+thumbnailList.get(_id));
				}
			} while (cur.moveToNext());
		}
		cur.close();
